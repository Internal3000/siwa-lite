...
class AAPLVSMSFT(DataFeed):
    ...
    @classmethod
    def detect_outliers_1(cls, ticker, market_caps, prev_data, threshold = 0.2):
        # Compare the single data point with previous data
        apis = market_caps.keys()
    ...
    @classmethod
    def detect_outliers_2(cls, ticker, market_caps, prev_data, threshold = 0.2):
        threshold_hit = False
        apis = market_caps.keys()
    ...
    def process_source_data_into_siwa_datapoint(cls):
        ...
        for ticker in tickers:
            ...
            if received == 3:
                ...
                outlier_removed_market_caps[ticker] = cls.detect_outliers_2(ticker, current_market_caps, prev_data)
            elif received == 1: 
                outlier_removed_market_caps[ticker] = cls.detect_outliers_1(ticker, current_market_caps, prev_data)
        
        for ticker in tickers: 
            received = len(outlier_removed_market_caps[ticker])
            count_str = colored(f'{received}/{total_sources}', 'light_magenta')
        ...
        cls.log(f"API results logged to {colored(file_path, 'blue')}")
